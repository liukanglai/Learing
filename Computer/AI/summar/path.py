...
class Must:
    ...
    def __init__(self, statue, start):
        self.statue = statue
        self.start = start
        # self.Env = env1.Env()  # class Env
    ...
    def permutation(self):
        ...
        for item in items:
            item0 = list(item)
            item0.insert(0, self.start)
            item0.insert(len(item0), self.start)
            items1.append(item0)
            len_item = len(item0)
            tem_distance = 0
            i = 0
            while i < len_item - 1:
                s_start = item0[i][0], item0[i][1]
                s_goal = item0[i + 1][0], item0[i + 1][1]
...
def main():
    ...
    if len(statue) > 1:
        ...
        print("distance:", tem_distance)
        plot = Plotting(start, new_statue)
        plot.animation(path, visited, "A*")  # animation

    else:
        s_goal = statue[0][0], statue[0][1]
        astar = AStar(start, s_goal, "euclidean")
        plot = Plotting(start, s_goal)

        path, visited = astar.searching()
        print("distance:", astar.get_value())
        plot.animation(path, visited, "A*")  # animation
