...
void *ReadMsgThreadPool(void *data) {
  webparam *parma = (webparam *)data;
  int fd = parma->fd;
  char *buffer = parma->buffer;
  int hit = parma->hit;
  long i, len;
  int j;
  ...
  index file */
    ...
    for (i = 0; extensions[i].ext != 0; i++) {
      ...
      if (!strncmp(&buffer[buflen - len], extensions[i].ext, len)) {
        parma->fstr = extensions[i].filetype;
        break;
      }
    }
    if (parma->fstr == 0)
...
void readFileThreadPool(void *data) {
  ...
  (void)sprintf(buffer,
                "http/1.1 200 ok\nserver: nweb/%d.0\ncontent-length: "
                "%ld\nconnection: close\ncontent-type: %s\n\n",
                VERSION, len, parma->fstr); /* header + a blank line */
  logger(LOG, "header", buffer, hit);
  (void)write(fd, buffer, strlen(buffer));
  parma->ret = read(file_fd, parma->file_buffer, FILEBUFSIZE); // > 0, success
  close(file_fd);
  SendMsgThreadPool(parma);
}

void SendMsgThreadPool(void *data) {
  webparam *parma = (webparam *)data;
  char *file_buffer = parma->file_buffer;
  int fd = parma->fd;
  (void)write(fd, file_buffer, parma->ret);
...
int main(int argc, char **argv) {
  ...
  /*threadpool *read_msg_pool = initReadMsgPool(8);*/
  /*threadpool *read_file_pool = initReadFilePool(8);*/
  /*threadpool *send_msg_pool = initSendMsgPool(8);*/
  for (hit = 1;; hit++) { // accept and create pthread
    ...
    /*web_task->function = ReadMsgThreadPool;*/
    /*web_task->next = NULL;*/
    ReadMsgThreadPool(param);

    /*addTask2ThreadPool(read_msg_pool, web_task);*/
  }
}
